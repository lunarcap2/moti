...
function fn_append_ct() {
	...
	for (var i = 0; i < arrCt1.length; i++) {
		if(arrCt1[i][0].substring(0,2) == ct1.substring(0,2)) {
			str += '<li><button type="button" class="button on" onclick="fn_append_ct2(this, \'' + arrCt1[i][0] + '\')">' + arrCt1[i][1] + '</button></li>';
...
function fn_append_ct2(obj, val) {
	...
	for (var i = 0; i < arrCt2.length; i++) {
		...
		str += '		<input type="checkbox" id="" class="chk" onclick="fn_appand_ctkey2(this); fn_append_ct3(this);" name="ctKeyWord2" value="'+ arrCt2[i][0] +'|'+ arrCt2[i][1] +'">'
...
function fn_ct3_set(ct2_val, ct2_nm) {
	...
	for (var i = 0; i < arrCt3.length; i++) {
		str_ct3 += '<li>'
		str_ct3 += '	<label class="checkbox off">'
		str_ct3 += '		<input type="checkbox" class="chk" name="ctKeyWord3" onclick="fn_appand_ctkey3(this, \''+ ct2_val +'\', \''+ ct2_nm +'\')" value="'+arrCt3[i][0] +'">'
		str_ct3 += '		<span>'+ arrCt3[i][1] +'</span>'
		str_ct3 += '	</label>'
		str_ct3 += '</li>'
	}
