...
function fn_append_jc() {
	...
	for (var i = 0; i < arrJc1.length; i++) {
		if(arrJc1[i][0].substring(0,2) == jc1.substring(0,2)) {
			str += '<li><button type="button" class="button on" onclick="fn_append_jc2(this, \'' + arrJc1[i][0] + '\')">' + arrJc1[i][1] + '</button></li>';
...
function fn_append_jc2(obj, val) {
	...
	for (var i = 0; i < arrJc2.length; i++) {
		...
		str += '		<input type="checkbox" id="" class="chk" onclick="fn_appand_jckey2(this); fn_append_jc3(this);" name="jcKeyWord2" value="'+ arrJc2[i][0] +'|'+ arrJc2[i][1] +'">'
...
function fn_jc3_set(jc2_val, jc2_nm) {
	...
	for (var i = 0; i < arrJc3.length; i++) {
		str_jc3 += '<li>'
		str_jc3 += '	<label class="checkbox off">'
		str_jc3 += '		<input type="checkbox" class="chk" name="jcKeyWord3" onclick="fn_appand_jckey3(this, \''+ jc2_val +'\', \''+ jc2_nm +'\')" value="'+arrJc3[i][0] +'">'
		str_jc3 += '		<span>'+ arrJc3[i][1] +'</span>'
		str_jc3 += '	</label>'
		str_jc3 += '</li>'
	}
...
function fn_jckey2_del(_val) {
	...
	$('#tab3 #col_ul_jc3 div').each(function() {
		if($(this).attr('id') == _val) {
			$('#' +_val + '').remove();
		}			
	});
}
